...
Version 2.5 - HERC Dashboard Upgrade and Health Metrics Expansion (April 2026) - Push v25
- Full HERC region dashboard rebuilt to mirror jurisdiction dashboard: added BSTA temporal risk table (regional averages across all member jurisdictions), all 7 PHRAT primary domain tiles (Air Quality, Dam Failure, and Vector-Borne Disease tiles were previously missing), category-header layout (Biological, Natural Hazards, Environmental, Security), supplementary domain tiles (Cybersecurity, Utilities) clearly labeled as not in PHRAT score, show-work popovers on every domain tile
- Added county-level communicable disease metrics from three new data sources: (1) Flu vaccination rate from County Health Rankings 2025 CSV, county-specific values 22%-69%; (2) Primary care physician access from CHR (9.6-173.8/100k); (3) COPD prevalence from CDC PLACES Socrata API (4.1%-10.2%). All integrated as EVR vulnerability/resilience modifiers.
- WI DHS WIR county immunization CSV now provides county-specific MMR rates for 24-month-olds, replacing statewide fallback
- Methodology page updated with accurate data source table entries for all three new health metrics sources
- Tribal practitioner email note added to jurisdiction selection page
