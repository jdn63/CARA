...
def register_blueprints(app):
    ...
    # Restricted to development environments only — never exposed in production.
    _is_dev = (
        os.environ.get("FLASK_ENV") == "development"
        or os.environ.get("REPLIT_DEV_DOMAIN") is not None
    )
    ...
    def mockup_proxy(path):
        if not _is_dev:
            return _Response("Not available in production", status=404)
