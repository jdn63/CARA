...
def generate_kaiser_hva_export(risk_data: dict) -> str:
    ...
    try:
        ...
        # Probability: derived from CARA score via _score_to_hva_scale (1-4).
        # Preparedness: derived inversely from CARA score via _score_to_preparedness —
        #   higher risk exposure implies less preparedness capacity (1-4).
        # Impact values are per-hazard-type defaults reflecting the nature of each
        # hazard; property_impact and business_impact remain constant because they
        # depend on hazard type more than jurisdiction-specific vulnerability.
        def _nh(key):
            return risk_data.get('natural_hazards', {}).get(key, 0.0)

        hva_hazards = [
            {
                'name': 'Flood',
                'cara_score': _nh('flood'),
                'probability': _score_to_hva_scale(_nh('flood')),
                'human_impact': 3,
                'property_impact': 4,
                'business_impact': 3,
                'preparedness': _score_to_preparedness(_nh('flood'))
            },
            {
                'name': 'Tornado',
                'cara_score': _nh('tornado'),
                'probability': _score_to_hva_scale(_nh('tornado')),
                'human_impact': 4,
                'property_impact': 4,
                'business_impact': 4,
                'preparedness': _score_to_preparedness(_nh('tornado'))
            },
            {
                'name': 'Winter Storm',
                'cara_score': _nh('winter_storm'),
                'probability': _score_to_hva_scale(_nh('winter_storm')),
                'human_impact': 2,
                'property_impact': 2,
                'business_impact': 3,
            ...
            },
            {
                ...
                'property_impact': 1,
                'business_impact': 4,
                'preparedness': _score_to_preparedness(risk_data.get('health_risk', 0.0))
            },
            {
                ...
                'property_impact': 2,
                'business_impact': 3,
                'preparedness': _score_to_preparedness(risk_data.get('active_shooter_risk', 0.0))
            },
            {
                'name': 'Extreme Heat',
                ...
                'property_impact': 1,
                'business_impact': 2,
                'preparedness': _score_to_preparedness(risk_data.get('extreme_heat_risk', 0.0))
            },
            {
                ...
                'property_impact': 1,
                'business_impact': 2,
                'preparedness': _score_to_preparedness(risk_data.get('air_quality_risk', 0.0))
            },
            {
                'name': 'Utility Outage',
...
def _score_to_preparedness(cara_score: float) -> int:
    """
    Derive a HVA preparedness rating from a CARA domain risk score.

    Higher CARA scores indicate greater risk exposure and, by extension,
    suggest less preparedness capacity relative to the threat level.
    This produces a preparedness value that is inversely correlated with
    the domain risk score, making the preparedness column jurisdiction-
    specific rather than a constant across all exports.

    Scale:
        1 = least prepared (highest risk)
        4 = most prepared (lowest risk)
    """
    if cara_score >= 0.75:
        return 1
    elif cara_score >= 0.50:
        return 2
    elif cara_score >= 0.25:
        return 3
    else:
        return 4