...
class RiskConfigManager:
    ...
    def get_em_overall_weights(self, jurisdiction_id: Optional[str] = None) -> Dict[str, float]:
        """
        Get Emergency Management discipline overall risk domain weights.

        These are stored under the 'em_overall_risk_weights' key in risk_weights.yaml,
        separate from the Public Health weights returned by get_overall_weights().

        Args:
            jurisdiction_id: Optional jurisdiction identifier for custom overrides

        Returns:
            Dictionary of EM risk domain weights
        """
        base_weights = self.config.get('em_overall_risk_weights', {})

        if jurisdiction_id:
            overrides = self.config.get('jurisdiction_overrides', {})
            if jurisdiction_id in overrides:
                override_weights = overrides[jurisdiction_id].get('em_overall_risk_weights', {})
                if override_weights:
                    weights = {**base_weights, **override_weights}
                    logger.info(f"Applied EM jurisdiction override weights for {jurisdiction_id}")
                    return weights
