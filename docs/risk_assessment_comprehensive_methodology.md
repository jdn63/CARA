...
| WI DHS WIR County Immunization | CSV download (dhs.wisconsin.gov/immunization/county-immunization-data.csv) | MMR (1) vaccination rate for 24-month-olds by county | Annual |
...
- Primary care physician density (CHR 2025, physicians per 100k). Range 9.6 (Forest) to 173.8 (Waukesha). Used as resilience modifier.
- COPD crude prevalence (CDC PLACES Socrata API, no key required). Range 4.1% (Waukesha) to 10.2% (Forest). Used as vulnerability modifier.
- MMR vaccination rate for 24-month-olds (WI DHS WIR county immunization CSV). County-specific; drives the strategic vaccination outbreak risk multiplier.
...
3. **Predictive Analysis**: The predictive analysis module (`utils/predictive_analysis.py`) uses a deterministic linear projection (trend of ±0.01/year converging toward 0.5) with fixed ±0.10 confidence intervals. The "historical" anchor values are derived from the current risk score minus a small offset. These are illustrative planning projections, not statistically modeled or empirically validated forecasts. They should not be cited as data-driven predictions.
...
- v2.5 push v25 (April 2026): Full HERC region dashboard rebuilt to mirror jurisdiction dashboard. Added BSTA temporal risk table, all 7 PHRAT primary domain tiles (Air Quality, Dam Failure, VBD were previously missing from HERC view), category-header layout, supplementary domain tiles, show-work popovers labeled as regional averages. County-level health metrics from County Health Rankings (flu vaccination, primary care access), CDC PLACES (COPD prevalence), and WI DHS WIR (MMR vaccination) integrated as EVR modifiers in disease domain.
- v2.5 push v24 (April 2026): County-level communicable disease metrics added via three new sources: CHR 2025 CSV (flu vaccination, primary care access), CDC PLACES Socrata API (COPD prevalence), WI DHS WIR CSV (MMR vaccination).
- v2.5 push v23 (March 2026): WI DHS WIR county immunization CSV replaces deprecated DHS GeoService COVID vaccination API. County-specific MMR rates now loaded for all 72 Wisconsin counties.
- v2.5 push v22 (March 2026): Tribal jurisdictions temporarily hidden from public selection pending data sovereignty protocol review. Fully reversible. No Tribal data or logic removed.
- v2.5 (February-March 2026): Real CDC/ATSDR SVI 2022 data for all 72 WI counties. Dam failure and VBD as separate PHRAT domains. EVR framework for natural hazards. Thread-safety fixes, concurrency capacity increase.
...
16. County Health Rankings 2025 (University of Wisconsin Population Health Institute) - flu vaccination rate, primary care physician access
17. CDC PLACES 2024 (Socrata API, data.cdc.gov/resource/swc5-untb.json) - COPD crude prevalence by county
18. Wisconsin Immunization Registry (WI DHS WIR) - county-level MMR vaccination rates for 24-month-olds
